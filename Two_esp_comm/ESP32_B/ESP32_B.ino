...
unsigned char *encrypted, *decrypted, *unpadding_decrypted;
;  // why we use unsigned char *?  1. because the  output of  mbedtls_aes_crypt_ecb() is unsigned char
...
void ThreadSensorEntry(void *pvParameters) {
  ...
  while (1) {
    ...
    if ((sample_error = dht11.read(&temperature, &humidity, NULL)) != SimpleDHTErrSuccess) {
      ...
      Serial.println("DHT11 Sample OK: ");
      ...
      if (Mode == 0) {
        ...
        mqttClient.publish("esp32_B/temperature", tempString);
        mqttClient.publish("esp32_B/humidity", humString);
        mqttClient.publish("dataESP2", (char *)plainText, (unsigned int)strlen(plainText));
        ...
        mqttClient.publish("encryptESP2", (char *)total_byte_stream, (unsigned int)totallength);
      }
...
void callback(char *topic, byte *message, unsigned int length) {
  ...
  if (Mode == 1) {
    ...
    Serial.print(strlen(Received));
    Serial.print("\n\ Received Encryption data length :");
    Serial.print(strlen(Receive_encrypted));
    Serial.print("\n\ Received Authentication data length :");
    Serial.print(strlen(Receive_authen));
    Serial.print("\n\ Received integrity data length :");
    Serial.print(strlen(Receive_integrity));
